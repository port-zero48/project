...
-- Allow all authenticated users to read withdrawal settings (they need this to see fees and admin account)
CREATE POLICY "Authenticated users can read withdrawal settings"
  ON public.withdrawal_settings
  FOR SELECT
  USING (auth.role() = 'authenticated');

-- Insert default row (single row design) - use DO block to avoid conflicts
DO $$
BEGIN
